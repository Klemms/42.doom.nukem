...
static inline void		my_pixel_put(t_doom *doom, int x, int y, int color)
{
	if (x >= 0 && y >= 0 && x < doom->w_width && y < doom->w_height)
		((unsigned int *)doom->img.img)[x + y * doom->w_width] = color;
}
...
static void				dx_dom(t_doom *doom, t_line *l)
{
	...
	while (++i <= l->dir.x)
	{
		...
		if ((l->inc.x > 0 && l->cur.x >= doom->w_width)
		|| (l->inc.x < 0 && l->cur.x < 0) || (l->inc.y < 0 && l->cur.y < 0)
		|| (l->inc.y > 0 && l->cur.y >= doom->w_height))
			break ;
		my_pixel_put(doom, round(l->cur.x), round(l->cur.y), l->color);
	}
}
